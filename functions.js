...
function addNewPoint() {
   let label
   let x1 = canvas.map(canvas.mouseX, 0, canvas.width, scale.x[0], scale.x[1])
   let x2 = canvas.map(canvas.mouseY, 0, canvas.height, scale.y[0], scale.y[1])
...
function drawPoints() {
   ...
   for (const point of points) {
      ...
      let x1 = canvas.map(point.x1, scale.x[0], scale.x[1], 0, canvas.width)
      let x2 = canvas.map(point.x2, scale.y[1], scale.y[0], canvas.height, 0)
...
function drawLine() {
   ...
   canvas.line(
      canvas.map(-1, scale.x[0], scale.x[1], 0, canvas.width),
      canvas.map(neuron.guessY(-1), scale.y[0], scale.y[1], 0, canvas.height),
      canvas.map(1, scale.x[0], scale.x[1], 0, canvas.width),
      canvas.map(neuron.guessY(1), scale.y[0], scale.y[1], 0, canvas.height)
   )
}
